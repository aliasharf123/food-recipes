        ...
        <AnimatePresence>
          {meals &&
            meals["meals"]
              ...
              .map((item, index) => <SingleMeal item={item} key={index} />)}
        </AnimatePresence>
      </div>
      <div className="items-center mx-auto mt-5 flex gap-3">
        {numberOfMeals < meals.meals.length && (
          <motion.button
            onClick={() => setNumberOfMeals(numberOfMeals + noOfCloumns)}
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.98 }}
            className="items-center bg-Theme px-4  font-medium rounded-sm py-2 mx-auto mt-5"
          >
            Show more{" "}
          </motion.button>
        )}
        {numberOfMeals > noOfCloumns * 2 && (
          <motion.button
            onClick={() => setNumberOfMeals(numberOfMeals - noOfCloumns)}
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.98 }}
